...
| 実施 | 版 |
|---|---|
| 10,000ノート負荷試験（AT-11の前半） | 1.13.4 |
| それ以外の実機テスト（AT-01〜AT-20） | 1.13.4 で初回、**1.13.7で全20シナリオを再実行して回帰なし** |
| 実APIテスト（AT-07 / AT-13） | **1.13.7** |

作業中にObsidianが**自動更新**されました（`obsidian-1.13.7.asar` がプロファイルに置かれ、
再起動で適用）。負荷試験の数値は1.13.4時点の記録として残し、それ以外は1.13.7で再確認しています。
...
| AT-14 | **合格** | `scripts/verify-release.mjs`: 版が `manifest.json`/`package.json`/`versions.json` で一致、`LICENSE`/`NOTICE` が存在、追跡76ファイルに鍵・メール・個人絶対パスなし。`npm run test:release:zip` が `release.yml` と同じ手順でzipを作り**中身が3ファイル**であることを検査。**2026-09-19に `v0.1.0` を公開**（GitHub Release、資産4件）。ワークフローが実際に走り、公開ステップの `GH_TOKEN` 不具合を1件検出して修正した | なし |
...
| RV-03 取消と通信枠 | **合格**（ソケット数は観測不能） | `requestUrl` を使わず `node:https` + `AbortController` で実装。**リダイレクトを追わず**、資格情報を他ホストへ転送しない。`test/jev.test.ts` で事前取消・取消中・長い`Retry-After` を検証。実機では4,531msで打ち切り、**取消を10回連打しても物理リクエストは1回**、期限後の応答は無視された | 残る物理ソケット数をJSから観測する手段がない。**観測できないことを「1本である」証拠にはしない** |
...
| RV-05 バッチ内だけの並べ替え | **解消** | Jevはローカル上位N件しか並べ替えないため、品質の上限は候補段階の再現率で決まる（RV-02の数値）。**Jev適用後のnDCG変化を実測**した: 公開コーパス806記事・カタカナ語クエリ120件で **nDCG@10 0.5258 → 0.8788**（改善75・悪化1・不変44）。ローカル段が候補をほぼ取り切っている（`matchedAnywhere` 100%）ため、この改善は**順序のみ**の改善である | 自然文クエリと実Vaultは未計測 |
