...
実機確認（2026-09-18、専用Vault・GUI E2E）: `setSecret` → 名前で解決 → `listSecrets` に出現 →
Obsidianを**正常終了して再起動**しても値が復元される。値はVault内にもプロファイル内にも**平文で見つからない**
（Windows資格情報マネージャの一覧にも該当項目なし）。

**保存範囲（実測・2026-09-19）**: 秘密は **Vault単位ではなくプロファイル単位**に保存される。
Vaultディレクトリを複製し、**別のプロファイル**で開くと `listSecrets()` は**空**を返した。
したがって「Vaultと一緒に運ばれる」「Vaultを共有すれば相手も使える」とは言えない。
同じプロファイルで**別のVault**を開いた場合の挙動は未確認である。

**制約（実測）**: SecretStorage の書き込みは**非同期にフラッシュされる**。`setSecret` 直後に強制終了
（`taskkill /F` 相当）すると値が失われた。**正常終了すれば保持される。** クラッシュ直後は再入力が必要になり得る。

未確認: 保存先の実体（OSキーチェーンかObsidian独自の保存か）、同期範囲、他プラグインや同一OSユーザーからの隔離。
