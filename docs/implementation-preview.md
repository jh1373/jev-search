...
- **Obsidianを正常終了して再起動しても保存キーが復元される**（値はVault内・プロファイル内に平文で見つからない）
- **レンダラ例外0件**
...
| 指標 | 変更前 | 変更後 |
|---|---|---|
| 索引完了までの時間（中央値） | 52.1s | **16.4s** |
| 「Ready」時点の索引数 | 6,400〜10,000件（不定） | **10,000件** |
| フレーム間隔 p95 | 17〜18ms | 20〜23ms |
| 200ms超のフレーム | 2 | 2 |
| JSヒープ | 144MB | 189〜251MB |

- UIは停止しませんでした。200msを超えたフレームは全実行で2回だけです。
- 1ノートごとのyieldを8msの時間予算に変えて3.2倍高速化しました（ADR-015）。p95は数ms悪化します。
- 計測中に**不具合を1件発見して修正**しました。「Ready」が索引完了前に立つことがあり、1回の実行では10,000件中6,400件で「Ready」と表示されました（ADR-016）。
- 詳細と限界は [result-load-10k.md](benchmark/result-load-10k.md)。
...
- キーは SecretStorage（値は data.json 外・Vault単位）とセッション上書きの二段階（ADR-013）。
  ...
  実機での保存・名前解決・再起動後の復元はGUI E2Eで検証済み（正常終了が必要。強制終了直後は失われ得る）。
